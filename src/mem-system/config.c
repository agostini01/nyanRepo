...
static void mem_config_read_signal_node(struct config_t *config,
		char *section, struct mod_t *mod)
{
	struct net_t *net;
	char *net_name;
	char *net_node_name_signal;
	struct net_node_t *net_node;

	/* High network */
	net_name = config_read_string(config, section, "HighNetwork", NULL);
	if (net_name)
	{
		net = net_find(net_name);
		assert(mod->high_net == net);
		net_node_name_signal = config_read_string(config, section,
				"HighNetworkNodeSignal", "");
		/* Get node */
		net_node = net_get_node_by_name(mod->high_net, net_node_name_signal);
		if (!net_node)
			fatal("%s: network %s: node %s: invalid node name for signal net -- high.\n%s%s",
					mem_config_file_name, net_name, net_node_name_signal,
					mem_err_config_note, err_mem_config_net);
		mod->high_net_node_signal = net_node;

		if (net_node->user_data)
			fatal("%s: network %s: node '%s' already assigned.\n%s",
					mem_config_file_name, net->name,
					net_node_name_signal, mem_err_config_note);
		net_node->user_data = mod;
	}

	/* Low network */
	net_name = config_read_string(config, section, "LowNetwork", NULL);
	if (net_name)
	{
		net = net_find(net_name);
		assert(mod->low_net == net);
		net_node_name_signal = config_read_string(config, section,
				"LowNetworkNodeSignal", "");
		/* Get node */
		net_node = net_get_node_by_name(mod->low_net, net_node_name_signal);
		if (!net_node)
			fatal("%s: network %s: node %s: invalid node name for signal net -- low.\n%s%s",
					mem_config_file_name, net_name, net_node_name_signal,
					mem_err_config_note, err_mem_config_net);
		mod->low_net_node_signal = net_node;
		...
		char *section)
{
	...
	/* MSHR record */
	mshr_record = config_read_bool(config, section, "RecordMSHR", 0);

	if (mshr_record)
	{
		mod->mshr_record = mod_mshr_record_create(mod);
	}
	/* Create cache */
...
static void mem_config_read_modules(struct config_t *config)
{
	...
	/* Reading Signal Section */
	for (section = config_section_first(config); section;
			section = config_section_next(config))
	{
		/* Section for a module */
		if (strncasecmp(section, "Extra ", 6))
			continue;

		/* Create module, depending on the type. */
		str_token(mod_name, sizeof mod_name, section, 1, " ");
		mod = mem_system_get_mod(mod_name);
		assert (mod && mod->kind == mod_kind_cache);
		mem_config_read_signal_node(config, section, mod);
	}
