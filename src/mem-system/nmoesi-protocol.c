...
void mod_handler_nmoesi_nc_store(int event, void *data)
{
	...
	if (event == EV_MOD_NMOESI_NC_STORE)
	{
		...
		/* Coalesce access */
		master_stack = mod_can_coalesce(mod, mod_access_nc_store, 
				stack->addr, stack);
		if (master_stack)
		{
	...
	if (event == EV_MOD_NMOESI_NC_STORE_WRITEBACK)
	{
		...
		if (stack->err)
		{
			...
			mem_debug("    lock error, retrying in %d cycles\n", 
					retry_lat);
			stack->retry = 1;
			esim_schedule_event(EV_MOD_NMOESI_NC_STORE_LOCK, stack,
					retry_lat);
			return;
		}
...
void mod_handler_nmoesi_prefetch(int event, void *data)
{
	...
	if (event == EV_MOD_NMOESI_PREFETCH_UNLOCK)
	{
		...
		/* DEV - PREFETCH set to zero latency instead of 1. why? Continue */
		esim_schedule_event(EV_MOD_NMOESI_PREFETCH_FINISH, stack, 0);
		return;
	}
...
void update_mod_stats(struct mod_t *mod, const struct mod_stack_t *stack)
{
...
void mod_handler_nmoesi_find_and_lock(int event, void *data)
{
	...
	if (event == EV_MOD_NMOESI_FIND_AND_LOCK_PORT)
	{
		...
		/* Set parent stack flag expressing that port has already been 
	...
	if (event == EV_MOD_NMOESI_FIND_AND_LOCK_ACTION)
	{
		...
		/* Continue */
		esim_schedule_event(EV_MOD_NMOESI_FIND_AND_LOCK_FINISH, 
				stack, 0);
		return;
	}
...
void mod_handler_nmoesi_evict(int event, void *data)
{
	...
	if (event == EV_MOD_NMOESI_EVICT_ACTION)
	{
		...
		stack->msg = net_try_send_ev(mod->low_net, src_node,
				dst_node, msg_size, EV_MOD_NMOESI_EVICT_RECEIVE, stack, event, stack);
		if (stack->msg)
	...
	if (event == EV_MOD_NMOESI_EVICT_RECEIVE)
	{
		...
		{
			new_stack = mod_stack_create(stack->id, target_mod, 
					stack->src_tag, EV_MOD_NMOESI_EVICT_PROCESS,
					stack);
		}
	...
	if (event == EV_MOD_NMOESI_EVICT_PROCESS)
	{
		...
		{
			ret->err = 1;
			esim_schedule_event(EV_MOD_NMOESI_EVICT_REPLY, 
					stack, 0);
			return;
		}
	...
	if (event == EV_MOD_NMOESI_EVICT_REPLY)
	{
		...
		stack->msg = net_try_send_ev(target_mod->high_net, src_node,
				dst_node, 8, EV_MOD_NMOESI_EVICT_REPLY_RECEIVE, stack,
				event, stack);
		if (stack->msg)
			net_trace("net.msg_access net=\"%s\" name=\"M-%lld\" access=\"A-%lld\"\n",
